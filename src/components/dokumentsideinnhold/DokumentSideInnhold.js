...
const hentGjelendeDokument = (journalposter, gjelendeJournalpostId) => {
  ...
  const gjelendeJournalpost = journalposter.filter(
    (journalpost) => journalpost.journalpostId === gjelendeJournalpostId
  )[0];
  if (!gjelendeJournalpost) {
    ...
    return gjelendeJournalpost.dokumenter.map((dokument) => (
      <ListeElement
        type="dokument"
        key={gjelendeJournalpost.journalpostId}
        journalpost={gjelendeJournalpost}
        dokument={dokument}
      />
    ));
  }
          ...
          <section id="dokumentliste">
            ...
            >
              {Dokument ? (
                ...
                <AlertStripeInfo>
                  Vi fant ikke noen dokumenter på dette id-nummeret. Ta <Lenke href={kontaktOssUrl}>kontakt</Lenke>{" "}
                  dersom det er noe du trenger hjelp med.
                </AlertStripeInfo>
              )}
              <Lenke
                className="se-alle-dokumenter"
                href={`${mineSakerUrl}/tema/${temakode}`}
